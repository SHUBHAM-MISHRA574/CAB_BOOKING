...
export default function BookCab() {
  ...
  const handleSeeDetails = () => {
    ...
    fetch(`${API_BASE}/api/locations/distance?${params}`)
      ...
      .catch(() => setError('Cannot reach server. Is the backend running? Set VITE_API_URL in client .env to your server URL (e.g. http://localhost:8080).'))
  ...
  const handleBook = async () => {
    if (!selectedCabId || !pickup?.display_name || !drop?.display_name || distanceKm == null || !scheduledAt) {
  ...
  return (
    <div className="flex min-h-[calc(100vh-64px)] w-full">
      ...
      <div className="hidden lg:flex lg:w-[55%] xl:w-[58%] flex-col bg-uber-black relative overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-20" style={{ backgroundImage: 'radial-gradient(circle at 20% 50%, #00C853 0%, transparent 50%), radial-gradient(circle at 80% 80%, #00C853 0%, transparent 40%)' }} />
        <div className="relative z-10 flex flex-col justify-between flex-1 p-10 xl:p-14">
          <div>
            <h1 className="text-3xl xl:text-4xl font-bold text-white tracking-tight">Where to?</h1>
          ...
          <div className="flex items-center gap-4 text-white/80">
            <div className="flex items-center gap-2">
              <div className="w-10 h-10 rounded-full bg-uber-green flex items-center justify-center text-uber-black font-bold">✓</div>
      ...
      <div className="flex-1 min-w-0 flex flex-col bg-neutral-50 lg:bg-white lg:border-l border-neutral-200">
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-lg mx-auto px-6 py-8 lg:py-10">
            <h2 className="text-2xl font-bold text-neutral-900 lg:hidden mb-2">Book a ride</h2>
            <p className="text-neutral-500 mb-6 lg:mb-8">Pick your locations and ride type.</p>
            ...
            {distanceKm != null && !distanceLoading && (
              <p className="mt-4 text-neutral-700 font-semibold">Distance: {distanceKm} km</p>
            )}

            {error && <p className="mt-4 text-red-600 text-sm">{error}</p>}
            {success && <p className="mt-4 text-uber-green font-semibold text-sm">{success}</p>}

            {/* Cab selection */}
            {distanceKm != null && distanceKm > 0 && (
              <>
                <h3 className="text-lg font-bold text-neutral-900 mt-8 mb-4">Choose a ride</h3>
                {cabsLoading ? (
                  <p className="text-neutral-500">Loading rides...</p>
                ) : cabs.length === 0 ? (
                  <div className="p-4 rounded-xl bg-neutral-100 text-neutral-600 text-sm">
                    No cabs available. Run <code className="bg-neutral-200 px-1 rounded">npm run seed</code> in the server folder.
                  </div>
                ) : (
                  <div className="grid grid-cols-2 gap-3 mb-6">
                    {cabs.map((cab) => {
                      ...
                      const imgUrl = cab.imageUrl || 'https://mobile-content.uber.com/launch-experience/top_bar_rides_3d.png';
                      return (
                        <button
                          key={cab._id}
                          type="button"
                        ...
                        >
                          ...
                          <div className="p-3">
                            <p className="font-semibold text-neutral-900 text-sm truncate" title={`${cab.type} – ${cab.name}`}>
                              {cab.type} – {cab.name}
                            </p>
                            <p className="text-xs text-neutral-500 mt-0.5">₹{cab.pricePerKm}/km · {cab.capacity} seats</p>
                            <p className="text-base font-bold text-uber-green mt-1.5">₹{fare}</p>
                          </div>
                        </button>
                      );
                    })}
                ...
                {cabs.length > 0 && (
                  <>
                    <div className="mb-4">
                      <label className="block text-sm font-medium text-neutral-700 mb-2">Date & time for pickup</label>
                      <DateTimePicker
                        id="pickup-datetime"
                        value={scheduledAt}
